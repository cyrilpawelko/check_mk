...
# For Backup Exec 12.5, download a adapted plugin from 
# Job must have run once to be inventoried
# Cyril Pawelko - cyril@pawelko.net http://www.pawelko.net
# Version 1.4
# To find the database instance and name, follow http://www.symantec.com/docs/TECH37752 until step 5
# If database connection error occurs, read http://www.symantec.com/docs/TECH24708
...
foreach ($job in $Jobs) {
  ...
  $jobId = $job.JobID
  $jobHistory = Invoke-SQL "select TOP 1 JobName,FinalJobStatus,EndTime,ElapsedTimeSeconds,TotalDataSizeBytes,TotalRateMbMin FROM vwJobHistorySummary2 WHERE (JobId = '$jobId' AND IsJobActive=0 ) ORDER BY ActualStartTime DESC"
  ...
  write-host job $jobname $jobHistoryResult.FinalJobStatus $jobHistoryResult.Endtime $jobHistoryResult.ElapsedTimeSeconds $jobHistoryResult.TotalDataSizeBytes $jobHistoryResult.TotalRateMbMin 0
}
